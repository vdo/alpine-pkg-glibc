...
package() {
  ...
  cp "$srcdir"/ld.so.conf "$pkgdir"/usr/etc/ld.so.conf
  rm "$pkgdir"/usr/etc/rpc
  rm -rf "$pkgdir"/usr/bin
  rm -rf "$pkgdir"/usr/lib/gconv
  rm -rf "$pkgdir"/usr/glibc-compat/lib/getconf
  rm -rf "$pkgdir"/usr/lib/audit
  rm -rf "$pkgdir"/usr/share
  rm -rf "$pkgdir"/usr/var
  ln -s /usr/lib/ld-linux-aarch64.so.1 ${pkgdir}/lib/ld-linux-aarch64.so.1
  ln -s /usr/lib/ld-linux-aarch64.so.1 ${pkgdir}/lib64/ld-linux-aarch64.so.1
  ln -s /usr/etc/ld.so.cache ${pkgdir}/etc/ld.so.cache
}
...
bin() {
  ...
  cp -a "$srcdir"/usr/bin "$subpkgdir"/usr
}
...
i18n() {
  ...
  arch="noarch"
  mkdir -p "$subpkgdir"/usr
  cp -a "$srcdir"/usr/share "$subpkgdir"/usr
}
